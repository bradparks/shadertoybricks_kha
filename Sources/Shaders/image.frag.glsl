...
float udHorizontalSegment( vec2 p, float xa, float xb, float y )
{
    vec2 pa = p - vec2(xa,y);
    float ba = xb - xa;
...
float SampleDigit(const float n, const vec2 vUV)
{
    if( abs(vUV.x-0.5)>0.5) return 0.0;
    if( abs(vUV.y-0.5)>0.5) return 0.0;

    // digit data by P_Mal(https://www.shadertoy.com/view/4sf3RN)
    float data = 0.0;
...
float PrintInt( vec2 uv, float value )
{
    float res = 0.0;
...
float _xglo;
vec2 _xcen;
float doBrick( vec2 id, vec3 col, float glo, vec2 cen )
{
    float alp = 0.0;
    ...
    if ( abs(id.y-5.5)<6.0 )
    {
        vec2 brickHere = texture2D( iChannel0, (txBricks.xy+id+0.5)/iChannelResolution0.xy, -100.0 ).xy;
...
void kore()
{
    ...
    fc.x *= iResolution.x;
    fc.y *= iResolution.y;
    vec2 uv = (-iResolution.xy + 2.0*fc.xy) / iResolution.y;
    float px = 2.0/iResolution.y;
    
    //------------------------
    // load game state
    //------------------------
    // vec2  ballPos   = texture2D( iChannel0, (txBallPosVel+0.5)/iChannelResolution0.xy ).xy;
    // float paddlePos = texture2D( iChannel0, (txPaddlePos+0.5)/iChannelResolution0.xy ).x;
    // float points    = texture2D( iChannel0, (txPoints+0.5)/iChannelResolution0.xy ).x;
    // float state     = texture2D( iChannel0, (txState+0.5)/iChannelResolution0.xy ).x;
    // vec3  lastHit   = texture2D( iChannel0, (0.5+txLastHit)/iChannelResolution0.xy ).xyz;
    vec2 ballPos = vec2(texture2D(iChannel0, (vec2(0.5, 0.5) / vec2(iChannelResolution0[0], iChannelResolution0[1])))[0], texture2D(iChannel0, (vec2(0.5, 0.5) / vec2(iChannelResolution0[0], iChannelResolution0[1])))[1]);
    float paddlePos = texture2D(iChannel0, (vec2(1.5, 0.5) / vec2(iChannelResolution0[0], iChannelResolution0[1])))[0];
    float points = texture2D(iChannel0, (vec2(2.5, 0.5) / vec2(iChannelResolution0[0], iChannelResolution0[1])))[0];
    float state = texture2D(iChannel0, (vec2(3.5, 0.5) / vec2(iChannelResolution0[0], iChannelResolution0[1])))[0];
    vec3 lastHit = vec3(texture2D(iChannel0, (vec2(4.5, 0.5) / vec2(iChannelResolution0[0], iChannelResolution0[1])))[0], texture2D(iChannel0, (vec2(4.5, 0.5) / vec2(iChannelResolution0[0], iChannelResolution0[1])))[1], texture2D(iChannel0, (vec2(4.5, 0.5) / vec2(iChannelResolution0[0], iChannelResolution0[1])))[2]);
